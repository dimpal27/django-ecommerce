...
from Admin.models import User, Category, Sub_Category, Product, gallery

from django.shortcuts import render, redirect, get_object_or_404
from django.contrib import messages
from datetime import date
from .form import OrderForm, CartItemForm
from Admin.form import UserForm
...
def client_login(request):
    if request.method == 'POST':
        ...
        val = User.objects.filter(email=e, password=p).count()
...
def client_register(request):
    if request.method == 'POST':
        ...
        abcd = UserForm()
    return render(request, 'client-register.html', {'form': abcd})
...
def payment_done(request):
    ...
    return render(request, 'order-success.html',
                  {'total_amount': total_amount, 'cart': cart, 'amount': amount, 'shipping_amount': shipping_amount})
